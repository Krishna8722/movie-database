...
Public Class Admin_Profile
    ...
    Private Sub Admin_Profile_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        Dim MysqlConn As New MySqlConnection(constr)
        Try
            MysqlConn.Open()
            Dim query As String
            query = "Select * from admin where username='" & Login.TextBox6.Text & "'"
            COMMAND = New MySqlCommand(query, MysqlConn)
            READER = COMMAND.ExecuteReader
            While READER.Read
                Dim id, name, mob, mail As String
            ...
            If IsPhoneNumberValid(TextBox4.Text) Then
                ...
                If (TextBox4.Text <> "" And TextBox2.Text <> "") Then
                    Try
                        MysqlConn.Open()
                        Dim Query As String
                        Query = "Update admin SET mobile='" & TextBox4.Text & "', email ='" & TextBox2.Text & "' WHERE username='" & TextBox16.Text & "'"
                        COMMAND = New MySqlCommand(Query, MysqlConn)
                        READER = COMMAND.ExecuteReader
